...
def load_log_vector(t,letter_vec, ngram):
	temp = "";
...
def get_predictions(string, array, lv):
	...
	for j in alphabet:
		text_mod = string+j;
		num = recover_frequency(lv, text_mod, array[len(text_mod)-1]);
		x.append((num, j));
...
def streaming_training(next_character, array1, array2, lv1, lv2, trellis,n):
	...
	nospace_val = recover_frequency(lv1, nospace, array1)+recover_frequency(lv2, nospace, array2)
	space_val = recover_frequency(lv1, space, array1)+recover_frequency(lv2, space, array2)
	# print " = "
	if (space_val>nospace_val):
		# print array1[len(nospace)-1]
		array1[len(nospace)-1] = array1[len(nospace)-1] + get_letter_vec(nospace, lv1)
		array2[len(nospace)-1] = array2[len(nospace)-1] + get_letter_vec(nospace, lv2)
		# print array1[len(nospace)-1]
	...
	for i in stripped_text[n-1:]:
		...
		for j in trellis:
			...
			if(not no_space in seen):
				seen.add(no_space)
				no_space_cost = recover_frequency(lv1, no_space, array1)+recover_frequency(lv2, no_space, array2)
			...
			if(not space_first in seen):
				...
				space_first_freq = recover_frequency(lv1, space_first, array1)+recover_frequency(lv2, space_first, array2)
...
try:
	...
	lv1 = random_idx.generate_letter_id_vectors(N,N/2);
	lv1.dump('log/lv_100k.p')
	postprocessed_array1 = np.array(load_ngram_vector(text, lv1, 6)) #creates the array up to 5 grams
...
try:
	...
	lv2 = random_idx.generate_letter_id_vectors(N,N/2);
	lv2.dump('log/lv_2_100k.p')
	postprocessed_array2 = np.array(load_ngram_vector(text, lv2, 6)) #creates the array up to 5 grams
...
def garden_path_accuracy(stripped_text, array, n, original):
	...
	for i in original[n-1:]:
		...
		for j in trellis:
			...
			no_space_cost = recover_frequency(lv1, no_space, array)
			space_first_freq = recover_frequency(lv1, space_first, array)
			space_second_freq = recover_frequency(lv1, space_second, array)
